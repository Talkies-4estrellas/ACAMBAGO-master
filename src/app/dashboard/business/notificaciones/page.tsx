...
import { Bell, ShoppingBag, MessageSquare, MessageCircle, Bookmark } from "lucide-react";
...
function iconFor(type: Notification["type"]) {
  ...
  if (type === "new_reservation") return Bookmark;
  return ShoppingBag;
}
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <Bell className="w-6 h-6 text-brand-600" /> Notificaciones
        </h1>
        <p className="text-gray-500 dark:text-slate-400 text-sm mt-0.5">Pedidos nuevos y preguntas de clientes</p>
      </div>

      {IS_DEMO ? (
        <div className="card p-10 text-center text-gray-400 dark:text-slate-500">Conecta Supabase para ver notificaciones reales.</div>
      ) : loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-slate-400">Todavía no tienes notificaciones.</p>
        </div>
      ) : (
            ...
            const content = (
              <div className={`px-5 py-3.5 flex items-start gap-3 ${n.link ? "hover:bg-slate-50 dark:hover:bg-white/5 transition-colors" : ""}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">{format(new Date(n.created_at), "dd MMM yyyy, HH:mm", { locale: es })}</p>
                </div>
